package io.nimbus.platform.gitops;
...
class ArgoSyncIntegrationTest {
    ...
    @Test
    void argoSyncIsSimulatedWithoutArgoCr() throws Exception {
        MvcResult login = mockMvc.perform(post("/api/v1/auth/dev-login")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"name":"Argo User","email":"argo@nimbus.local"}
                                """))
        ...
        MvcResult project = mockMvc.perform(post("/api/v1/projects")
                        ...
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"name":"Argo Project","workspaceId":"%s"}
        ...
        AppService service = AppService.create(
                projectId, UUID.fromString(workspaceId), "argo-api", "svc",
        ...
        mockMvc.perform(get("/api/v1/services/{id}/argo-sync", service.getId())
                        .header("Authorization", "Bearer " + token))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.mode").value("SIMULATED"))
                .andExpect(jsonPath("$.data.applicationManifest").value(
                        org.hamcrest.Matchers.containsString("kind: Application")))
                .andExpect(jsonPath("$.data.serviceName").value("argo-api"));
    }
}
