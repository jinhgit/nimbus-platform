package io.nimbus.platform.pipeline;
...
@ActiveProfiles("test")
class GitHubActionsRunIntegrationTest {
    ...
    @Test
    void githubRunsAreSimulatedWithoutScm() throws Exception {
        MvcResult login = mockMvc.perform(post("/api/v1/auth/dev-login")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"name":"Pipe User","email":"pipe-gh@nimbus.local"}
                                """))
        ...
        MvcResult project = mockMvc.perform(post("/api/v1/projects")
                        ...
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"name":"Pipe Project","workspaceId":"%s"}
        ...
        AppService service = AppService.create(
                projectId, UUID.fromString(workspaceId), "pipe-api", "svc",
        ...
        mockMvc.perform(get("/api/v1/pipelines/github-runs")
